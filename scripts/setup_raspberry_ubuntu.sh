...
usage() {
  cat <<'EOF'
Usage: ./scripts/setup_raspberry_ubuntu.sh [options]
...
Prepares an Ubuntu Raspberry Pi for this OpenMower ROS workspace:
  ...
  - hardens boot/login to never use UART console (or run
    ./scripts/disable_uart_console.sh standalone)
  - applies GPS/NTRIP defaults early (or run
    ./scripts/configure_gps_ntrip_defaults.sh standalone)
...
configure_uart_for_mega_bridge() {
  ...
  local disable_uart_console_script="${SCRIPT_DIR}/disable_uart_console.sh"
  if [[ ! -f "${uart_script}" ]]; then
    echo "Missing UART setup script: ${uart_script}" >&2
    exit 1
  fi
  if [[ ! -f "${disable_uart_console_script}" ]]; then
    echo "Missing UART console hardening script: ${disable_uart_console_script}" >&2
    exit 1
  fi
...
install_wifi_service() {
  ...
  local wifi_script="${REPO_ROOT}/scripts/connect_wifi_from_env.sh"
